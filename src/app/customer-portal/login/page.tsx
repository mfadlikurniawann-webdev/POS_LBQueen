...
export default function CustomerLoginPage() {
  ...
  return (
    <div className="min-h-screen bg-[#FDFCFD] font-sans relative flex flex-col items-center selection:bg-rose-100 selection:text-[#C94F78]">
      {/* ── PREVIEW BANNER ── */}
      {isAdminPreview && (
        <div className="fixed top-0 left-0 right-0 bg-amber-500 text-white text-[10px] font-semibold uppercase tracking-[0.2em] py-2 px-4 z-[100] text-center flex items-center justify-center gap-2 shadow-md">
      ...
      <div className="w-full h-[40vh] lg:h-[45vh] relative overflow-hidden flex flex-col items-center justify-center shrink-0 bg-rose-50 border-b border-rose-100">
        ...
        <div className="relative z-10 flex flex-col items-center mt-[-40px]">
          ...
          <h1 className="text-[#C94F78] text-2xl font-semibold tracking-tight drop-shadow-sm flex items-center gap-2">
            Portal Pelanggan <Crown className="w-5 h-5 text-amber-400" />
          </h1>
          <p className="text-[#C94F78]/60 text-[11px] font-semibold uppercase tracking-[0.2em] mt-1.5 drop-shadow-sm">
            Care Beauty
          </p>
        </div>
      </div>
      ...
      <div className="w-full max-w-[400px] px-5 -mt-20 lg:-mt-24 relative z-20 pb-12">
        <div className="bg-white rounded-[32px] shadow-2xl shadow-rose-100/60 border border-white p-7 lg:p-8 relative overflow-hidden">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <p className="text-[10px] text-slate-400 font-semibold mt-1.5 px-1 leading-relaxed">
              💡 Default: <span className="text-[#C94F78]">INISIAL-STATUS-ID</span> (cth: SW-M-5)
            </p>

            <button type="submit" disabled={loading}
              className="w-full py-4 bg-[#C94F78] hover:bg-[#A83E60] text-white font-semibold text-[14px] rounded-2xl flex items-center justify-center gap-2 transition-all duration-300 shadow-luxury-pink active:scale-[0.98] disabled:opacity-70 mt-2">
